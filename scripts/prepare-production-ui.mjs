...
const ORIGINAL_MARKER = 'PITCHLINE_PRODUCTION_UI_TRANSFORMED';
const alreadyTransformed = source.includes(ORIGINAL_MARKER);
...
if (!alreadyTransformed) {
  ...
  source = `/* ${ORIGINAL_MARKER} */\n${source}`;
  await fs.writeFile(file, source, 'utf8');
  console.log('Pitchline production UI transformed: fallback demo state removed.');
} else {
  console.log('Pitchline production UI already transformed; skipping duplicate source mutation.');
}
