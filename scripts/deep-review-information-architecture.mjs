...
check('live operators include Manager and Club roles', backend.includes("['LFA Admin','Manager','Club']") || backend.includes("['LFA Admin','Manager','Club Manager']"));
check('live event route uses scoped operator access', backend.includes("'/api/live-match/events-v2'") && backend.includes('scopedFixtureAccess'));
check('live event duplicate detection remains', backend.includes('findDuplicateLiveEvent'));
check('live match concurrency lock remains', backend.includes('acquireLiveLock'));
...
check('public live report is supporter-safe', backend.includes('verifiedAt:verification?.verifiedAt') && !backend.includes('discipline:discipline.filter(d=>d.fixtureId===params.fixtureId)'));
check('standings recalculation is competition-scoped', backend.includes('recalcStandings(String(next.competitionId||fixture.competitionId||\'\')||undefined)'));
check('safeguarding case notes persist', backend.includes('ownerRole:assignment.role,note,createdAt:now,updatedAt:now'));
check('invoice overdue status is date-aware', backend.includes("new Date().toISOString().slice(0,10)"));
check('competition bracket team validation resolves registered teams', backend.includes('registeredTeams.find(t=>String(t.id)===ref||String(t.name)===ref)'));
...
if (failures.length) { console.error(`INFORMATION ARCHITECTURE AUDIT: FAIL (${failures.length})`); process.exit(1); }
