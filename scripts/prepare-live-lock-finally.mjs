...
const acquireStart = api.indexOf("  const liveMutationRoutes = ['/api/live-match/events-v2','/api/live-match/events','/api/live-match/undo','/api/live-match/pause','/api/live-match/resume','/api/live-match/finish'];");
...
if (!api.includes('LIVE_LOCK_FINALLY_GUARD')) {
  ...
  const cleanedAfterDuplicate = afterDuplicate.replace(earlyRelease, '');
  ...
  // Idempotently keep ownership stable if the source was already wrapped.
  api = api.replace(ownerLine, stableOwnerLine);
}

fs.writeFileSync(apiPath, api);
console.log('Hardened every live-match mutation with a true try/finally lock-release guard.');
