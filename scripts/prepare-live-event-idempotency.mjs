...
const duplicateGuard = "if(locked)return error('Verified matches are locked',409);const eventKey=liveEventKey(input);const existingEvents=await listTable<LiveEvent>('live_events');const duplicate=existingEvents.find(event=>event.fixtureId===fixtureId&&liveEventKey({fixtureId:event.fixtureId,type:event.type,minute:event.minute,clockSeconds:event.clockSeconds,team:event.team,player:event.relatedPlayer||event.player,playerOffRef:(event as Record<string,unknown>).playerOffRef,playerOnRef:(event as Record<string,unknown>).playerOnRef,note:event.note,goalDetail:(event as Record<string,unknown>).goalDetail,assistRef:(event as Record<string,unknown>).assistRef})===eventKey);if(duplicate)return json({...live,events:existingEvents.filter(event=>event.fixtureId===fixtureId).sort((a,b)=>a.createdAt-b.createdAt)});const team=";
