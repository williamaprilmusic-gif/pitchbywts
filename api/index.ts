...
function requestIp(request: VercelRequest) {
  ...
  return String(value || request.headers['x-real-ip'] || '').split(',')[0].trim().slice(0, 100) || undefined;
}

async function writeAudit(record: AuditRecord) {
  try { await db.add('audit_log', [{ ...record, createdAt: record.createdAt || Date.now() }]); }
...
async function isLfaAdmin(userId?: string) {
  ...
  return roles.items.some(item => String(item.userId) === userId && String(item.role) === 'LFA Admin');
}

async function auditEndpoint(request: VercelRequest, response: VercelResponse, reqId: string) {
  const user = getSessionUser(request);
  if (!user || !(await isLfaAdmin(user.userId))) {
    send(response, 403, { error: 'LFA Admin role required', message: 'LFA Admin role required' }, reqId);
    return true;
  }
  if (request.method !== 'GET') {
    send(response, 405, { error: 'Method not allowed', message: 'Method not allowed' }, reqId);
    return true;
  }
  ...
  const filtered = (items.items || [])
    .filter(item => !pathFilter || item.path.includes(pathFilter))
    .sort((a, b) => Number(b.createdAt || 0) - Number(a.createdAt || 0));
  send(response, 200, { items: filtered, count: filtered.length }, reqId);
  return true;
}
...
function normalizePath(request: VercelRequest) {
  ...
  return new URL(request.url || '/', 'https://pitchline.local').pathname;
}
  ...
  try {
    if (pathname === '/api/auth/sign-in' && request.method === 'POST') {
      ...
      if (!user) {
        ...
        if (!found || !verifyPassword(password, String(found.passwordHash || ''))) return send(response, 401, { error: 'Invalid email or password.', code: 'invalid_credentials' }, reqId);
        user = { userId: String(found.id), email, name: String(found.name || email) };
      }
      setSessionCookie(response, user);
      return send(response, 200, { user, accessToken: '', expiresIn: 60 * 60 * 24 * 7 }, reqId);
    }
    if (pathname === '/api/auth/sign-up' && request.method === 'POST') {
      ...
      const users = await db.list<Record<string, unknown>>('auth_users', { limit: 5000 });
      if (users.items.some(item => String(item.email || '').toLowerCase() === email)) return send(response, 409, { error: 'An account already exists for that email.', code: 'account_exists' }, reqId);
      const [id] = await db.add('auth_users', [{ email, name, passwordHash: passwordHash(password), createdAt: Date.now() }]);
      if (!id) return send(response, 500, { error: 'Could not create account.', code: 'account_create_failed' }, reqId);
      ...
      setSessionCookie(response, user);
      return send(response, 201, { user, accessToken: '', expiresIn: 60 * 60 * 24 * 7 }, reqId);
    }
    if (pathname === '/api/auth/me' && request.method === 'GET') {
      try {
        const user = getSessionUser(request);
        if (!user) return send(response, 401, { error: 'Unauthorized', code: 'not_authenticated' }, reqId);
        return send(response, 200, { user }, reqId);
      } catch (error) {
        return send(response, 503, { error: error instanceof Error ? error.message : 'Authentication service is not configured.' }, reqId);
      }
    }
    if (pathname === '/api/auth/sign-out' && request.method === 'POST') {
      clearSessionCookie(response);
      return send(response, 200, { ok: true }, reqId);
    }
    return false;
  } catch (error) {
    console.error('Pitchline auth error', error);
    return send(response, 500, { error: error instanceof Error ? error.message : 'Authentication service failed.' }, reqId);
  }
}
...
export default async function api(request: VercelRequest, response: VercelResponse) {
  ...
  if (pathname === '/api/_healthcheck' && request.method === 'GET') {
    return send(response, 200, { ok: true, runtime: 'vercel-node', databaseConfigured: Boolean(process.env.DATABASE_URL), sessionConfigured: Boolean(process.env.PITCHLINE_SESSION_SECRET), adminBootstrapConfigured: Boolean(process.env.PITCHLINE_ADMIN_EMAIL && process.env.PITCHLINE_ADMIN_PASSWORD), auditLoggingEnabled: Boolean(process.env.DATABASE_URL), timestamp: new Date().toISOString() }, reqId);
  }
  if (pathname === '/api/audit-log') {
    await auditEndpoint(request, response, reqId);
    return;
  }
  const authHandled = await authEndpoint(request, response, pathname, reqId);
