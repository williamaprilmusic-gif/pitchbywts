import { db, ws, json, error } from '../server/appdeployCompat';
...
export async function notifySubscribers(entityType: string, entityId: string, payload: unknown, excludeConnectionId?: string) {
  const items = await listSubscriptions();
  const targetConnectionIds = Array.from(new Set(
    items.filter(item => item.entity_type === entityType && item.entity_id === entityId)
